...
**⚠️ 嚴格字數限制 — 超過會導致旁白與畫面嚴重錯位，這是硬性限制。**

### 旁白字數上限（每段 section）

| Section | 場景時長 | 字數上限 | 範例 |
|---------|---------|---------|------|
| OPENING（開場）| ~4s | **15 字** | 「今天帶你看這間信義區精裝兩房」(15字) |
| 一般空間 clip | 2.5s | **10 字** | 「客廳採光超好格局方正」(10字) |
| MAP（周邊） | 10s | **40 字** | |
| STATS（規格） | 7s | **28 字** | |
| CTA（聯繫） | 5s | **20 字** | |

> 計算基準：1.2x 語速 ≈ 4.8 字/秒，再預留 20% 安全餘量。
> **停頓標記 `<#秒數#>` 不算字數，但會佔用時間，有停頓的 section 要再少 2-3 字。**

**自我檢查（必做）**：寫完講稿後，逐段數字數（不含標點和停頓標記）。超過上限 → 刪減到上限內。一個字都不能超過。
