...
```
[OPENING]
（1-2 句 hook，點出地點優勢或最大賣點，接著帶入附近 POI 生活機能，例如最近的捷運站、超市、公園等，讓觀眾感受到地段便利性）
...
- **嚴禁**編造 `raw_text` 沒有提到的空間細節（例如：陽台、落地窗、中島、浴缸等）
- 如果某個空間在 `raw_text` 中沒有具體描述，用通用且安全的說法（如「空間明亮」「動線順暢」「格局方正」）
...
**重要：講稿長度決定影片時長。** 語速約 **4 字／秒**，字數直接控制每段鏡頭長度。各段目標：
- OPENING：約 8-10 秒（32-40 字）— 先用 1-2 句 hook 抓注意力，再補充地段優勢或生活機能
- 每個空間：約 4-6 秒（16-24 字）
- STATS：約 6-8 秒（24-32 字）
- CTA：約 4-6 秒（20-28 字）

**總字數參考公式**：`(空間數 × 20) + 100` 字（例如 3 個空間 → 約 160 字）
