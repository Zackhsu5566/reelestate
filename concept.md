...
- 「修改資料」指令可重新填寫
- 每次 job 最多上傳 15 張照片（超過提示已達上限）
- 歡迎訊息提示「建議上傳 10 張以內照片，效果最佳」
- 物件資訊提示「可以直接貼上房屋介紹文字，AI 會自動擷取重點」
...
Split by [MARKER] → 每段 section 各自呼叫 MiniMax T2A (sync t2a_v2)
     ...
     → 若旁白超過場景時長 → 自動拉長場景 durationInFrames
     → silence padding 對齊 + concat → narration.mp3 + aligned subtitles
     → 講稿文字 s2twp 轉繁體（避免簡體顯示在 LINE）
     → 上傳 R2
     ↓ 失敗 → 降級（無旁白繼續）
...
| 常數 | 值 | 時長 |
|------|---|------|
| CLIP_FRAMES | 105 | 3.5s（1.25x 速率） |
| CLIP_SMALL_FRAMES | 84 | 2.8s（小空間） |
...
### 動態場景時長
若某段旁白 TTS 音訊超過該場景的預設時長，`extend_scenes_for_audio()` 會自動拉長 `durationInFrames`，
並將覆寫值存入 `JobState.scene_duration_overrides`，render 時套用。旁白較短時維持原本長度。
...
| 字幕 | sentence-level，底部居中半透明黑底白字，fade in/out，16 字自動換行 |
...
- ✅ Per-scene TTS alignment + silence padding（2026-03-26）
- ✅ LINE gate emoji 分段預覽 + 雙向編輯（2026-03-26）
- ✅ 客廳/廚房 camera 改為 Pull Out（2026-03-26）
- ✅ Audio alignment race condition 修復（2026-03-27）
- ✅ MAP scene 加長至 15s（2026-03-27）
- ✅ SubtitleOverlay 16 字自動換行（2026-03-27）
- ✅ MapScene 房屋 marker 改為 🏠 emoji，zoom 13（2026-03-28）
